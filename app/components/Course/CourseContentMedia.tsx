...
import {
  ...
  useCompleteUserProgressMutation,
...
import CoursePlayerPrivate from "../Player/CoursePlayerPrivate";
...
import { ConvertSecondToLabel, converTimeHrToSec } from "@/app/utils/dateFomat";
...
type Props = {
  ...
  refetchUserProgress: any;
  user: any;
  refetch: any;
  totalWatched: number;
  totalLimitWatch: number;
};
...
const CourseContentMedia = ({
  ...
  const { updateWatchTime, } = useVideoWatchTime(id)

  const isOverWatchTime = totalWatched >= converTimeHrToSec(totalLimitWatch);


  useEffect(() => {
    if (!responseUpdateWatched?.course_id && id) {
      updateWatchTime(1)
    }
  }, [])
  ...
  const handleQuestion = () => {
    if (question.length === 0) {
      ...
      addNewQuestion({
        question,
        courseId: id,
        contentId: videoList[activeVideo]._id,
      });
    }
  };
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      socketId.emit("notification", {
        ...
        message: `You have a new question in ${videoList[activeVideo].title}`,
        userId: user._id,
      });
    }
    if (answerSuccess) {
  ...
  const handleCompleteVideo = () => {
    const currentVideo = videoList[activeVideo]
    ...
    completeUserProgress({
      ...
      if (activeVideo < videoList.length - 1) {
  ...
  const remainingTime = converTimeHrToSec(totalLimitWatch) - totalWatched
  ...
  return (
    <div className="w-full sticky top-0 md:max-w-[1000px]">
      {
        isOverWatchTime ? <>
          <div className="w-full text-center min-h-[20vh] space-y-5">
            <h1 className="text-2xl">คุณหมดโคต้าในการรับชมแล้ว</h1>
            <p className="text-md">คุณรับชมไปทั้งหมด {ConvertSecondToLabel(responseUpdateWatched?.total_watch_time || 1)}</p>
          </div>
        </>
          :
          <CoursePlayerPrivate
            ...
            videoUrl={videoList?.[activeVideo]?.videoUrl}
            onUpdateTime={(time) => setCurrentTime(time)}
          />
      }
      ...
      <div className="w-full flex items-center justify-between my-3">
        ...
        {
          remainingTime > 0 && <div>
            <span className="text-sm text-gray-400">สามารถเรียนได้อีก {ConvertSecondToLabel(remainingTime)}</span>
          </div>
        }
        <div
          className={`${styles.button
            } !w-[unset] text-white  !min-h-[40px] !py-[unset] ${videoList?.length - 1 === activeVideo && "!cursor-no-drop opacity-[.8]"
            }`}
          onClick={() =>
            setActiveVideo(
              videoList && videoList?.length - 1 === activeVideo
                ? activeVideo
                : activeVideo + 1
            )
          }
      ...
      {activeBar === 0 && (
        <p className="text-[18px] whitespace-pre-line mb-3 dark:text-white text-black">
          {videoList?.[activeVideo]?.description}
        </p>
      )}

      {activeBar === 1 && (
        <div>
          {videoList?.[activeVideo]?.links.map((item: any, index: number) => (
            <div className="mb-5" key={index}>
              <h2 className="800px:text-[20px] 800px:inline-block dark:text-white text-black break-words">
                {item.title && item.title + " :"}
              </h2>
              <a
                className="text-[#4395c4] 800px:text-[20px] 800px:pl-2 break-words"
                href={item.url}
              >
                {item.url}
              </a>
